...
Private Sub chkconviction_Click()
    If EditorIndex = 0 Or EditorIndex > MAX_SPELLS Then Exit Sub
Spell(EditorIndex).Conviction = chkconviction.Value

End Sub

Private Sub chklycanthropy_Click()
    If EditorIndex = 0 Or EditorIndex > MAX_SPELLS Then Exit Sub
Spell(EditorIndex).Lycanthropy = chklycanthropy.Value
End Sub

Private Sub chkmagic_Click()
    If EditorIndex = 0 Or EditorIndex > MAX_SPELLS Then Exit Sub
Spell(EditorIndex).Magic = chkmagic.Value
End Sub
...
Private Sub txtspellexp_Change()
    ' If debug mode, handle error then exit out
    If options.Debug = 1 Then On Error GoTo errorhandler
    
    If Not Len(txtspellexp.text) > 0 Then Exit Sub
    If IsNumeric(txtspellexp.text) Then Spell(EditorIndex).EXP = Val(txtspellexp.text)
