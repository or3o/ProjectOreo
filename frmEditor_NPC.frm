...
Private Sub txtskillexp_Change()
    ' If debug mode, handle error then exit out
    If options.Debug = 1 Then On Error GoTo errorhandler
    
    If Not Len(txtskillexp.text) > 0 Then Exit Sub
    If IsNumeric(txtskillexp.text) Then NPC(EditorIndex).Skillexp = Val(txtskillexp.text)
